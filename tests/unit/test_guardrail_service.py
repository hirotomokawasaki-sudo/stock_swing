...
def test_etf_buys_disabled_by_default(tmp_root: Path, monkeypatch) -> None:
...
def test_etf_position_multiplier_is_035(tmp_root: Path, monkeypatch) -> None:
    monkeypatch.delenv("PAPER_DEMO_ALLOW_ETF_BUYS", raising=False)
    result = get_guardrail_status(tmp_root)
    assert result["etf_position_multiplier"] == pytest.approx(0.35, abs=0.01)
