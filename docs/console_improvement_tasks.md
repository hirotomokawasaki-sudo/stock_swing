...
**09-08実施レビュー結果（第2回、事前固定基準 `docs/review_criteria_20260908.md` §1 に照合）**:
- **サンプル未達**: entry>=08-14 の stop_loss は n=11 fills / 6論理トレードのまま
  09-05から増加ゼロ（最後の stop_loss exit は 08-31、9月に入り発生0件）。
  基準 n>=15〜20 未達のため、事前固定ガードに従い閾値 [0.5, 1.75] は**現状維持**。
- **直近コホート寄与（新観点、exit>=08-14 の 9論理トレード全件列挙）**: baseline
  （固定 -5/-7/-9%）反実仮想比で**正味 +$1,067**（min_hold 相互作用を考慮した
  感度分析では約 +$2,260）→ 「+$1,000超なら現状維持を明確に支持」を満たす。
- **⚠️ widen起因の悪化事例が初観測（LRCX 2件、08-14/08-17ロット、mult≈1.49/1.35）**:
  widen された閾値が exit を 08-31 まで遅延させ、naive 計 -$1,634
  （min_hold 調整後 約 -$438）の悪化。ギャップ支配ではない真の widen 遅延。
  中止条件（3件以上かつ -$5,000 超）は未達。
- **tighten 改善は継続**: QTUM +$659 に加え NVDA +$1,384 / CHPX +$246 /
  ASML +$162 / AVGO 2件 +$310 を確認（tighten 7件合計 約 +$2,700）。
- **記録した提案**: 次回レビューで `volatility_multiplier_max` 1.75→**1.25** への
  引き下げを正式評価する（max=1.25 なら LRCX 2件は 08-28 exit となり widen 悪化を
  解消、tighten 側の改善は全て保持）。変更前に
  `simulate_daily_path_volatility_stop.py` での全履歴再検証（新規誤発動チェック含む）
  を必須とする。
- 次回レビュー: **2026-09-22 09:00 JST**（lot_level_exit 次回判断期限と同日）。
  ⚠️ **cronは未登録**（制限付きautomation runからは新規登録不可。メインセッション
  からの登録と実在確認が必要 — 過去2回の未登録インシデントの再発防止）。
- 詳細: `docs/daily_logs/2026-09-08.md`
...
- ATR閾値のmultiplier範囲（0.5〜1.75）・sector_shockのrolling閾値（-5.0%）は
  初期値。09-08レビューで widen 悪化2件（LRCX）が初観測され、multiplier_max
  1.75→1.25 引き下げ提案を記録済み（09-22レビューで全履歴再検証とともに正式評価）
...
**👤 ユーザー決定（2026-09-08）**: Plan C の paper_ab 昇格は**見送り**。
最新再集計（n=18、5勝13敗、負け比率72.2%、合計PnL -$13,617）でも事前条件は
満たしたが、true率34.6%と対象が広く勝ちトレードも遮断するトレードオフを踏まえ、
現時点ではshadow観測を継続する。**実装・設定変更は行わない**。
...
### 初回shadowログ観測結果（2026-09-08, 2026-08-25〜09-04 US）

- **総件数**: `data/dip_buy_meanreversion_shadow_log.jsonl` **625件**
  （33 run / 稼働9営業日、1 runあたり6〜39件）。
- **日次件数**: 08-25: 49 / 08-26: 40 / 08-27: 43 / 08-28: 31 / 08-31: 44 /
  09-01: 29 / 09-02: 148 / 09-03: 128 / 09-04: 113。09-02以降の約3倍増は
  9月頭の半導体・ソフトウェア売りによるもので、「下げ相場で候補が増える」
  という設計どおりの挙動。
- **銘柄分布**: 48銘柄。上位: AVGO 33 / INTC 33 / AMAT 33 / ARM 32 /
  KLAC 30 / PTF 25 / PSCT 25 / DDOG 22 / CSCO 21。半導体個別株+
  半導体ETF系（SMHX/SOXQ/SOXX/FTXL/CHPS等）に集中しており、universe構成と
  8月末〜9月頭のセクター下落に整合。
- **momentum分布**: min -42.2% / median -9.0% / mean -10.9% / max -5.0%
  （エントリー閾値≤-5%と整合、閾値ぎりぎりの張り付きなし）。
  バケット: -5〜-7%: 172件 / -7〜-10%: 197件 / -10〜-15%: 141件 /
  <-15%: 115件。
- **signal_strength分布**: min 0.400 / median 0.521 / mean 0.568 / max 1.0。
  バケット: 0.4-0.5: 253件 / 0.5-0.6: 169件 / 0.6-0.7: 88件 / ≥0.7: 115件。
- **蓄積の健全性**: 稼働日はrun（13:25/13:35/16:00/19:55 UTC）ごとに毎回
  ログ蓄積を確認。dip_buy関連のWARNING/ERRORは`data/audits/paper_demo_*.log`
  および`logs/`配下に**0件**（best-effortパスの
  `dip_buy_meanreversion shadow check failed`警告も未出現）。唯一の欠落は
  **09-01の3 run（13:25/13:35/16:00 UTC）**だが、audit logで
  `paper_demo_start`自体が欠落しており、同日のpaper_demo実行欠落
  （09-01/09-02の既知インシデント）が原因でdip_buyシャドー配線の問題では
  ない。09-05〜09-07の欠落は週末+Labor Day休場で正常。
- **暫定判断（観測記録のみ、本番接続の判断はしない）**: 件数はPlan Bの
  初週目安（5件以上）を大幅に超えて充足し、収集パイプラインは健全。
  ただし08-26追記の修正前提（「chop検知連動型」への再設計）+ Gate 3
  strategy-scoping + 資本配分分離が未着手のため、**shadow継続**。
  次回レビュー（3〜4週間後のpromotion判断、09-22〜09-29目安）では、
  shadowシグナルの反実仮想リターン照合（signal後5〜10営業日の実価格 vs
  `SimpleExitV2Strategy`想定exit）を実施し、特に09-02〜09-04の大量候補
  （高ボラ局面のdip）が実際に反発したかを確認すること。
- ⚠️ 次回レビューcronは本レビュー実行セッション（automation run）の権限
  制約で登録不可。メインセッションから
  `stock_swing_r14_dip_buy_promotion_review_20260922`相当の登録が必要。
...
### 📋 2026-09-08 初回レビュー結果（cron `stock_swing_lot_level_exit_diagnostic_review_20260908`）

**1) 蓄積状況**: `data/lot_level_exit_shadow_log.jsonl` = **3件**、全て2026-09-01T19:55Zの
単一run由来。内訳: `aggregate_exit_lot_disagreement` **3件**（NOW/ORCL/PLTR）、
`aggregate_missed_lot_exit` **0件**（診断の当初動機であるNOW型「希釈による発火見送り」は
未観測——09-01のmax_hold全量決済でポジション自体が先に消えたため）。
09-02以降ログ0件だが配線は健全: PATHが3ロット（min_lots=2充足）で毎run評価対象に
なっており、ログ無し＝ロット判定と集約判定が一致（discrepancyのみ記録する設計通り）。

**2) 実値動きとの反実仮想評価（per-lot判定なら09-01に売られなかった若ロット3本）**:

| ロット | 実際(集約exit) | per-lot反実仮想（実価格ベース） | 差分 |
|---|---|---|---|
| ORCL 340株@149.17 | -$2,768 (@141.03) | 保有継続→09-03/04に急反発（09-04終値158.78、高値159.7でtrailing活性化）。trailing(~153.3)またはper-lot max_hold(~09-08)で **+$1,400〜+$3,300** | **+$4,200〜+$6,000 有利** |
| NOW 385株@148.84 | -$2,333 (@142.78) | 保有継続。stop(-10.07%→$133.85)未到達（最安136.38）、09-04終値141.26で含み損-$2,918、未決済 | **-$585 不利**（現時点） |
| PLTR 281株@175.43 | +$1,334 (@180.18) | 保有継続→09-02に急落（終値169.46）。peak186.7のtrailing(~179.2)をギャップ割れ、09-02決済で **≈-$1,700** | **≈-$3,000 不利** |

合算では per-lot が +$600〜+$2,400 有利だが、**ORCLの偶発的な反発1本に完全依存**して
おり方向は混在。n=1イベント・3ロットでは「per-lot判定が損益面で優る」とは言えない。
ただし重要な観察: 3本とも反実仮想経路で per-lot 自身のstop_loss床を一度も割っておらず、
「若ロットを保有継続しても無防備ではない」（per-lot保護は機能する形状）ことは確認できた。

**3) 昇格判断: 観測継続（shadow維持）**
- **フルのロット単位exit評価への昇格（paper_ab）は見送り**。根拠: サンプルn=1イベント、
  損益方向が混在、`aggregate_missed_lot_exit`（本命の希釈型）は未観測のまま。
  promotion_governance.mdの基準を満たすエビデンスがない。
- **`per_lot_time_based_exit_enabled`（09-02実装、default OFF）は有効化推奨**。これは
  価格予測ではなく「若ロットを他ロットの時計で強制決済しない」構造是正で、若ロットは
  自前のstop/trailing/max_holdで保護され続ける（上表の通り反実仮想でも保護床は無傷）。
  ロールバックはフラグ1行。**ただし本番挙動変更のためユーザー承認待ち**（無人レビューでは
  フラグを触らない）。
- 次回中間レビュー: **2026-09-22目安**。⚠️ cron再登録は本レビューrun（automationセッション）からは
  権限上不可だったため未登録。メインセッションから
  `stock_swing_lot_level_exit_diagnostic_review_20260922` として要登録。
  観測ポイント: `aggregate_missed_lot_exit`の初観測有無、PATH等の多ロット銘柄での
  discrepancy発生、（有効化された場合）per_lot_time_based_exitの部分売却実績。
...
### R18-D: 多重検定/オーバーフィット管理（起票 → **発効済み 2026-09-05**）

同一closed 357件に対する反実仮想の重ね掛け（セクター上限・ss帯除外・early weakness
cut・R18-A・R18-B、以後も増える）は多重検定であり、最良に見える施策は過大評価
されている前提で扱う。

**2026-09-05 発効（ユーザー承認 06:27）**: `docs/promotion_governance.md` として
必須条件化した。要点:
- 反実仮想は**探索的**位置づけ（昇格の根拠にならない）。
- 本番反映（paper A/B昇格を含む挙動変更）の必須3条件: **(a)判断基準の事前登録
  (b)paper A/Bでの前向き確認 (c)R13-C完了後のout-of-sample再検証**（完了前に
  昇格したものは遡及再検証、不合格なら差し戻し）。
- **検証台帳（Verification Ledger）**を同文書に新設（初版8本: 08-14
  volatility_adjusted_stop、08-23 R13-A stop深掘り・R13-B ss exit切り離し、
  09-05 early weakness cut・ss帯縮小・セクターcap・R18-A・R18-B）。以後の
...
## R20: 下落局面の武器(ロングオンリー制約下、2026-09-05起票)

**制約の確定(2026-09-05ユーザー確認)**: 取引制限は**ショート(空売り)のみ禁止**。それ以外は可能 — インバースETFのロング買付・オプション買い等は制限対象外。

**候補(優先順、すべて発注前にshadow/検証→promotion_governance.mdの必須3条件に従う)**:

1. **R20-A ディフェンシブ・ローテーション(本命)**: R13-D(sector_rotation)のユニバースに守備資産 TLT(米長期債)/GLD(金)/XLP/XLU(ディフェンシブ株)/BIL(短期債≒現金) を追加。「リスクオフ=守備資産へ回転」を既存機構のまま実現。新規機械不要でshadow検証も既存枠組みを流用可能。R13-D本体の配線判断(9/8)後に着手。
2. **R20-B ボラティリティ・ターゲティング**: R18-C(ATR定額サイジング)の口座全体版。実現ボラ上昇時に総エクスポージャを自動縮小。検証はR18-C と統合。
3. **R20-C 現金化の段階ルール**: 月次実現損の階層(例: -$50k到達で新規サイズ半減)。既存circuit_breakerの上位レイヤー。shadow検証は軽量。
4. **R20-D インバースETFの戦術的ロング(可能と確定)**: SH/PSQ等の非レバレッジ型を優先。レバレッジ型(SQQQ/SOXS等)は日次リバランス減価が大きく短期戦術限定。**裁量発動は禁止** — 発動条件をルール化しshadow→paper A/Bを経ること。減価・トラッキング誤差の定量化を検証項目に含める。
5. **R20-E プロテクティブ・プット(中期)**: オプション買いによる損失限定ヘッジ。ブローカーのオプション権限・保険料ドラッグの定量化が前提。IBKR移行後に再評価。

**備考**: R20-A/B/Cはショート不可制約と無関係に有効な守備策であり優先。R20-Dは相場急変時の即応手段として価値があるが、減価コストと発動誤爆リスクがあるため4番手。

---

## R21: stale価格補正のセッション認識ガード（2026-09-08）

**背景 / インシデント**: 2026-09-04（金）の米国市場中、PATHのbrokerライブ価格が
約$15.23まで急落した一方、Massive日足の最新値は前営業日09-03終値$18.22だった。
`compute_stale_price_overrides()`は価格乖離だけでbroker価格をstaleと判定したため、
ライブ価格を前日終値で上書きし、-12% emergency stopの判定を一時的に隠した。

**修正**:
- US pre-market / regular / after-hours中は、日足barの日付が現在のETセッション日と
  一致する場合だけoverride生成を許可する。前日以前・日付不明のbarは正常skip扱い。
- 市場休場中は従来どおり直近営業日の日足を利用可能とし、週末・祝日の補正用途を維持。
- 生成時だけでなく、`data/price_overrides.json`から保存済みoverrideを再適用する
  `SimpleExitV2Strategy`側も同じガードを通す。前日のファイルが残っていても
  ライブセッション中のstop判定を隠せない。

**検証**: PATH実データ形状の回帰テストに加え、pre-market / regular /
after-hours、同日bar許可、週末の前営業日bar許可、保存済みoverrideの日付不明拒否を
固定テスト化。対象・関連テスト114件、フルスイート2311 passed / 2 skipped。
